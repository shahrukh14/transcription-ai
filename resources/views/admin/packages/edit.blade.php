                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-4 mb-1">
                            ...
                            <input type="text" class="form-control" placeholder="@lang('Enter Package Name')" name="name" id="name" value="{{$package->name}}" required>
                        </div>
                        <div class="col-md-4 mb-1">
                            <label for="amount" class="form-label">@lang('Amount')<span class="text-danger">*</span></label>
                            <input type="number" step="any" class="form-control" placeholder="@lang('Enter Package Amount')" name="amount" id="amount" value="{{$package->amount}}" required>
                        </div>
                        <div class="col-md-4 mb-1">
                            <label for="type" class="form-label">@lang('Package Type')<span class="text-danger">*</span></label>
                            <select name="type" id="type" class="form-control" required>
                                <option value="monthly" @selected($package->type == "monthly")>Monthly</option>
                                <option value="yearly" @selected($package->type == "yearly")>Yearly</option>
                            </select>
                        </div>
                        <div class="col-md-4 mb-1">
                            <label for="audio_time_limit" class="form-label">@lang('Audio Time Limit')<span class="text-danger">*</span></label>
                            <input type="number" class="form-control" placeholder="@lang('Enter Audio time limit')" name="audio_time_limit" id="audio_time_limit"  value="{{$package->audio_time_limit}}" required>
                        ...
                        <div class="col-md-4 mb-1">
                            ...
                            <input type="number" class="form-control" placeholder="@lang('Enter Daily Transcription Limit')" name="transcription_limit" id="transcription_limit" value="{{$package->transcription_limit}}" required>
                            <span class="text-warning">Enter 0 (Zero) for unlimited transcription.</span>
                        </div>
                    </div>
                  
                    <div class="descriptionAddMoreDiv mb-1">
                        @php
                        ...
                        <div class="row add-more-div">
                            <div class="col-md-1 mb-1">
                                <img src="{{asset("admin/packages/descriptions/".$desc->image)}}" alt="descriptions" class="object-cover-fit" width="100" height="100">
                            </div>
                            <div class="col-md-3 mb-1">
                               <label for="description[{{$key}}][image]" class="form-label">@lang('Image')</label>
                               <input type="file" class="form-control"  name="description[{{$key}}][image]" id="description[{{$key}}][image]">
                               <input type="hidden"  name="description[{{$key}}][old_image]" value="{{$desc->image}}">
                           </div>
                           <div class="col-md-4 mb-1">
                               <label for="description[{{$key}}][title]" class="form-label">@lang('Title')</label>
                               <input type="text" class="form-control" placeholder="@lang('Enter Description Title')" name="description[{{$key}}][title]" id="description[{{$key}}][title]" value="{{$desc->title}}">
                           </div>
                           <div class="col-md-4 mb-1">
                               <label for="description[{{$key}}][content]" class="form-label">@lang('Content')</label>
                               <input type="text" class="form-control" placeholder="@lang('Enter Description Content')" name="description[{{$key}}][content]" id="description[{{$key}}][content]" value="{{$desc->content}}">
                           </div>
...
<script>
    $(document).ready(function() {
        ...
        let lastKey = "{{$key}}";
        let descriptionaddMoreCounter  = parseInt(lastKey) + 1;
